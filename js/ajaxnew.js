...
(function($){
	...
	getmoreboard=function(x){//中央版控制--x==動態牆上之選項....y=人的id
		if(sessionStorage.getItem("member")){ //20190422 Pman 修正當member不存在的時候，會不能往下讀取的問題
			...
			if(data[0]=="ERR"){
				...
				if(me=="notice"){//開單一動態/攻略/qna
					for(var a=0;a<data[1].length;a++){
						...
						}else if(parseInt(data[1][a]['typeid'])==9){
							out+="                              <div class='formdivdivon formdivdiv submenuclick' data-val='7'>\n";
						}else{//純通知類
	...
	setsize=function(){
		...
		$("#popbody").css("max-height",2*$(window).height()/5);
		$("#popbodyu").css("max-height",2*$(window).height()/5);
		thisval=$("#mainwrap").offset();
		if(window.page=="share.php"){
			//$("#mainwrap").css("margin-top",45); //20190219 Pman 修正shape.php這頁的元件位置
			$("#headeroutwrap").css("height",45);
			$("#mainright").css("height",$(window).height()-45-60);
			$("#mainleft").css("height",$(window).height()-45-60);
			$("#mainleftin").css("height",$(window).height()-45-290);
			//$("#mainright").css("top",45); //20190219 Pman 修正shape.php這頁的元件位置
			//$("#mainleft").css("top",45); //20190219 Pman 修正shape.php這頁的元件位置
		}else{
				...
				if(data[0]=="ERR"){
					...
					if(data[1].length>0){
						for(var a=0;a<data[1].length;a++){
							...
							out+="                     	</div>\n";
							out+="                     </div>\n";
							out+="                     <div class='frienditemtext'><span>"+data[1][a]['name']+"</span></div>\n"; //20180904 Pman
