...
$(document).ready(function() {
	...
	var fbemail="";
	var fbacc=0;	
	var chattime=10000;//chatroom refresh---這個設定和後台通知有連動..因此不能直接改
	var refreshnote=10000;//檢查訊息時間
	var refreshtimer=60000;//檢查訊息時間
	var xxme="";
	...
	$.getScript('js/sharenew.js', function() {	
		//alert("PP01");
		$.getScript('js/ajaxnewtest.js', function() {	
				alert("POS3");
			//匯入store.js
			...
			//中央
			$.getScript('js/centertest.js', function() {
				cntss--;
			});	
			//右方
			...
			if($(this).data("val")==1){
				...
				tempitem.success(function(data){
					if(data[0]=="ERR"){
						...
						out+="                        <div class='formitem formitem_3 tleft'>累積 Energy</div>\n";
						out+="                        <div class='formitem formitem_3 dtext'>"+data[1]['score']+"</div>\n";
						out+="                        <div class='clr'></div>\n";
						out+="                    </div>\n";
						out+="                    <div class='formline2'>\n";
						out+="                        <div class='formitem formitem_3 tleft'>剩餘 Energy</div>\n";
						out+="                        <div class='formitem formitem_3 dtext'>"+data[1]['points']+"</div>\n";
						out+="                        <div class='clr'></div>\n";
						out+="                    </div>\n";
						out+="                    <div class='formline2'>\n";
						out+="                        <div class='formitem formitem_3 tleft'>近30日紀錄</div>\n";
						out+="                        <div class='formitem formitem_3'>\n";
						out+="							<span class='span1'>訂單號碼</span>";
						out+="							<span class='span2'>點數</span>";
						out+="							<span class='span3'>點數名</span>";
						...
						for(var a=0;a<data[2].length;a++){
							...
							if(parseInt(data[2][a]['orderid'])>100){
								out+="							<span class='span1'>"+data[2][a]['orderid']+"</span>";
							}else{
								out+="							<span class='span1'>NA</span>";
							}
     ...
     fjs.parentNode.insertBefore(js, fjs);
		...
		$("body").delegate(".aboutclick","click",function(e){
			...
			if(x!="gamedel"){
				...
				$("#"+x+"form").val(myval);
				if(x=="name" && myval.length>0 ){//暱稱---這整個都應該刪除了
		...
		$("body").delegate(".maintemselect","click",function(){
			...
			}else if(thisval==3){//這個有回復及主體
				if(window.confirm("確認要刪除嗎?提醒您,刪除後將無法回復")) { 
					var tempvals=Array(sessionStorage.getItem("userid"),sessionStorage.getItem("key"),thisval,thistype,thisid);// 類別,ID
					tempitem=ajaxarr("reactions",tempvals,"ajax.php");
					tempitem.success(function(data){//回傳 data 義	
						if(data[0]=="ERR"){
		...
		$("body").delegate(".bgip,.qnap ","click",function(){
			...
			if(me.hasClass("noclick")){//disable
				...
				}else if(window.confirm("確認支持這個答案?")){
					var tempvals=Array(sessionStorage.getItem("userid"),sessionStorage.getItem("key"),me.data("type"),me.data("key"),me.data("id"));// 類別,ID
					tempitem=ajaxarr("givepoint",tempvals,"ajax.php");
					tempitem.success(function(data){//回傳 data 義	
						if(data[0]=="ERR"){
		...
		$("body").delegate(".submitclick","click",function(e){
				...
				if($(this).data("type")=="orderform"){
						...
						var tempvals=Array(sessionStorage.getItem("userid"),sessionStorage.getItem("key"),mylistb.eq(0).val(),mylistb.eq(1).val(),mylistb.eq(2).val(),mylistb.eq(3).val(),mylistb.eq(4).val(),radioval,mylistb.eq(5).val(),mylistb.eq(6).val());
							...
							tempitem.success(function(data){//回傳 data 義	
								...
								if(data[0]=="ERR"){
									...
									show_afterloginhead();
									$("#maincontentwrap").html("<div class='responsetext'>您的訂購已經成立,謝謝</div>");
						...
						if(sessionStorage.getItem("userid")){
							...
							tempitem.success(function(data){//回傳 data 義	
								if(data[0]=="ERR"){
									...
									if(tempme.height()>205){
										tempme.css("height",205);
					...
					if(articles.length<30){
						popnotice("攻略請最少寫數十字,謝謝");
						passme=0;
					}else if(articles.length>100000){
