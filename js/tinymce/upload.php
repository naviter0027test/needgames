...
#######################################
	function upnews(){
		...
		if($hasimg==1){
				...
				/*更新結束*/
				$fileext="jpg"; //20190305 Pman 因為上傳轉檔後，都是轉成jpg，所以將副檔名都改為jpg
				echo "<script>\n";
				echo "$(document).ready(function() {\n";
				//echo "$('.mce-placeholder',parent.document).val('uploadfile/".$tempid.".".$fileext."');\n";
				//echo "$('.mce-floatpanel',parent.document).css('opacity',1);\n";
				echo "$('.mce-textbox',parent.document).eq(0).val('uploadfile/".$tempid.".".$fileext."');\n";
				echo "$('.mce-floatpanel',parent.document).css('opacity',1);\n";
