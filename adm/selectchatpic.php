...
function sorting($mytable,$myid,$mypage){
	global $pdos;
	$neworder="";
	$temp=share_gettable($pdos,$mytable." WHERE typeid='".$GLOBALS['typeid']."'  ORDER BY sorting DESC limit ".($GLOBALS["myorder"]-1).",1");
	if($temp){
		$neworder=$temp[0]['sorting'];
	}
	if($GLOBALS["thisorting"]>$neworder){
		share_update($pdos,$mytable,"sorting=sorting+1"," typeid='".$GLOBALS['typeid']."' AND sorting<".$GLOBALS["thisorting"]." AND sorting>=".$neworder);
	}else{
		share_update($pdos,$mytable,"sorting=sorting-1"," typeid='".$GLOBALS['typeid']."' AND sorting>".$GLOBALS["thisorting"]." AND sorting<=".$neworder);
	}
...
function upcontent($mytable,$myid,$mypage){
	...
	global $thumbheight;	
     if( empty($GLOBALS["thisname"])){
		...
		if($myerr){
			...
			$temp=share_gettable($pdos,$mytable." WHERE typeid='".$GLOBALS['typeid']."' order by sorting DESC limit 0,1");
			if(count($temp)>0){
				$mysort=$temp[0]["sorting"]+1;
			}else{
				$mysort=1;
			}
			if($t=share_insert($pdos,$mytable,"thisname,typeid,isopen,sorting","'".$GLOBALS["thisname"]."','".$GLOBALS["typeid"]."','".$isopen."','".$mysort."'")){
 ...
 function getcontent($mytable,$myid,$mypage){
	...
	$types=share_gettable($pdos,"chatimgtype ORDER BY sorting DESC");
	echo "<h2>".$GLOBALS['mytitle']."設定</h2>\n";
	echo "<TABLE class=formtable width=800>";
	echo "<FORM action='index.php?tf=".$mypage."' method='post'>\n";
	echo "<TR><TH>選擇分類</TH><TH><select name='typeid'>\n";
	foreach($types as $t){
		echo "<option value='".$t['thisid']."' ".($t['thisid']==$typeid?"SELECTED":"")." >".$t['thisname']."</option>\n";
	}
	echo "</SELECT></TH><TH><input type=submit class='gray-btn'></TH></TR>\n";
	echo "</FORM>\n";
	echo "</TABLE>\n";
	if($typeid){
		 echo "<TABLE class=formtable width=800>";
		 echo "<tr><FORM action='index.php?tf=".$mypage."&job=addcontent&typeid=".$typeid."' method='post' id='addcontentform' enctype='multipart/form-data'>\n";
		 echo "<TR><th colspan=4>新增</th></TR>\n";
		 echo "<TR><th class=req style='width:120px'>檔案名稱</th><th>縮圖</th><th>使用</th><th>新增</th></TR>\n";
		 ...
		 echo "<TR><th style='width:70px'>刪除</th><th class=req style='width:120px'>檔案名稱</th><th>圖片(png)</th><th>使用</th><th>更新</th><th>排序</th></TR>";
		 $rows=share_gettable($pdos,$mytable." WHERE typeid='".$typeid."' ORDER BY sorting DESC");
		 $z=0;
		 foreach($rows as $row){
				 echo "<tr><FORM action='index.php?tf=".$mypage."&job=upcontent&id=".$row['thisid']."&typeid=".$typeid."' method='post'  enctype='multipart/form-data' ><td><a href='index.php?tf=".$mypage."&job=delcontent&id=".$row['thisid']."' class='blue-btn'>刪除</a></td>\n";
				...
				echo "<FORM action='index.php?tf=".$mypage."&job=changesorting&thisorting=".$row["sorting"]."&id=".$row['thisid']."&typeid=".$typeid."' method='post' ><td><select name=myorder class=changeselect>\n";
