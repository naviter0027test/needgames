...
#######################################
	...
	function upcontent($mytable,$mypage){
		...
		if($myerr){
			...
			//新增或更新
			if($GLOBALS["id"]){
				share_update($pdod,$mytable,"url='".$GLOBALS['url']."',gameid=".($GLOBALS['gameid']?$GLOBALS['gameid']:"null")." ,isopen='".$isopen."',isnew='".$isnew."',startdate='".$GLOBALS["startdate"]."',enddate='".$GLOBALS["enddate"]."'","bannerid=".$GLOBALS["id"]);
				$tempid=$GLOBALS["id"];
			}else{
				share_insert($pdod,$mytable,"typeid,url,gameid,isopen,isnew,startdate,enddate","'".$GLOBALS['typeid']."','".$GLOBALS['url']."',".($GLOBALS['gameid']?$GLOBALS['gameid']:"null").",'".$isopen."','".$isnew."','".$GLOBALS["startdate"]."','".$GLOBALS["enddate"]."'");
			...
			if(is_uploaded_file($_FILES["file"]['tmp_name'])){
				...
				//createimg($tempid,$imgurl,$width,$height,$imgwidth,$imgheight,$imgs,$thumbwidth,$thumbheight,$src,$fileext,0,100);		//這裡有調整寬度設定 //20190307 Pman 品質設為100，上傳不壓縮 //20190308 Pman 客戶要求圖檔完全不壓縮，所以，上傳後直接改名另存
			}
			if($GLOBALS["id"]){
	...
	function getcontent($mytable,$mypage){
		...
		$games=share_gettable($pdom,"gametype");
			if($GLOBALS['typeid']){
				if($_SESSION['lanid']){		
				 ...
				 echo "<tr><th colspan=8>新增內容(輪播圖)</th>";
				 echo "<TR><th >圖片</th><th  style='width:150px' >連結</th><th  style='width:80px' >分類</th><th  style='width:60px' >新視窗</th><th  style='width:60px' >使用</th><th  style='width:80px' >開始日</th><th  style='width:80px' >結束日</th><th style='width:80px'>新增</th></TR>";
				 ...
				 echo "<td><select name='gameid'><option value=''>不設定</option>";
				 foreach($games as $g){
					 echo "<option value='".$g['typeid']."'>".$g['typename']."</option>";
				 }
				 echo "</select></td>";
				 ...
				 echo "</TABLE>";
				 echo "<TABLE class=formtable width=800>";
				 echo "<TR><Th colspan=9 >".$GLOBALS['mytitle']."內容管理</Th></TR>";
				 echo "<TR><th style='width:60px'>刪除</th><th >圖片</th><th  style='width:150px' >連結</th><th  style='width:80px' >分類</th><th  style='width:60px' >新視窗</th><th  style='width:60px' >使用</th><th  style='width:80px' >開始日</th><th  style='width:80px' >結束日</th><th style='width:80px'>更新</th></TR>";
				 ...
				 foreach($newslist as $news){
						...
						echo "<td><select name='gameid' style='width:80px;'><option value=''>不設定</option>";
						foreach($games as $g){
							 echo "<option value='".$g['typeid']."'";
							 if($g['typeid']==$news['gameid']){
								 echo " selected ";
							 }
							 echo ">".$g['typename']."</option>";
						 }
						 echo "</select></td>";
						if($news['isnew']==1){
							echo "<td><input type=checkbox name=isnew value='1' checked></TD>";
