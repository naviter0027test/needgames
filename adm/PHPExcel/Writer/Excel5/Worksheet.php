...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	 ...
	 * @param integer $col1line   Start column
	 * @param integer $row2   End row
	 * @param integer $col2   End column
	 * @param string  $url	URL string
	 * @return integer
	 */
	function _writeUrlRange($row1, $col1line, $row2, $col2, $url)
	{
		// Check for internal/external sheet links or default to web link
		if (preg_match('[^internal:]', $url)) {
			return($this->_writeUrlInternal($row1, $col1line, $row2, $col2, $url));
		}
		if (preg_match('[^external:]', $url)) {
			return($this->_writeUrlExternal($row1, $col1line, $row2, $col2, $url));
		}
		return($this->_writeUrlWeb($row1, $col1line, $row2, $col2, $url));
	}
	 ...
	 * @return integer
	 */
	function _writeUrlWeb($row1, $col1line, $row2, $col2, $url)
	{
		...
		$header	  = pack("vv",   $record, $length);
		$data		= pack("vvvv", $row1, $row2, $col1line, $col2);
	 ...
	 * @return integer
	 */
	function _writeUrlInternal($row1, $col1line, $row2, $col2, $url)
	{
		$record	  = 0x01B8;					   // Record identifier
		$length	  = 0x00000;					  // Bytes to follow
	 ...
	 * @return integer
	 */
	function _writeUrlExternal($row1, $col1line, $row2, $col2, $url)
	{
		// Network drives are different. We will handle them separately
		// MS/Novell network drives and shares start with \\
		if (preg_match('[^external:\\\\]', $url)) {
			return; //($this->_writeUrlExternal_net($row1, $col1line, $row2, $col2, $url, $str, $format));
		}
