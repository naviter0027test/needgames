...
//		$volatile = $this->_functions[$token][3];
	...
	function _convertRange2d($range, $class=0)
	{
		...
		list($row1, $col1line) = $this->_cellToPackedRowcol($cell1);
		...
		return $ptgArea . $row1 . $row2 . $col1line. $col2;
	}
...
//		$class = 0; // formulas like Sheet1!$A$1:$A$2 in list type data validation need this class (0x3B)
		...
		if (preg_match("/^(\\$)?[A-Ia-i]?[A-Za-z](\\$)?(\d+)$/", $cell1)) {
			...
			list($row2, $col2) = $this->_cellToPackedRowcol($cell2);
			 ...
			 list($row1, $col1line, $row2, $col2) = $this->_rangeToPackedRange($cell1.':'.$cell2);
		}
...
//		}
		...
		return $ptgArea . $ext_ref . $row1 . $row2 . $col1line. $col2;
	}
...
//		}
	 ...
	 * @return array Array containing (row1,col1line,row2,col2) in packed() format
	 */
	function _rangeToPackedRange($range)
	{
		...
		$col1line |= $row1_rel << 15;
		$col2 |= $row2_rel << 15;
		$col1line = pack('v', $col1line);
		$col2 = pack('v', $col2);

		$row1 = pack('v', $row1);
		$row2 = pack('v', $row2);

		return array($row1, $col1line, $row2, $col2);
	}
