<?php
		...
		if($x[0]==$_SESSION['userid'] && $x[1]==$_SESSION['key'] && $x[2]<>"" && $x[3]<>""){//確認資格
			...
			//	$out[1]="目前貢獻值不足,無法扣點,請調整貢獻值額後再試";
				...
				if($x[4] && $x[5]=="1"){//圖片
					...
					$temp=share_gettable($pdo,"pho_ WHERE albid='".$x[4]."'");//這個和動態共用了所以存到pho
					$tout="";
					foreach($temp as $t){
						$tout.="<img src=uploadfile/".$t['thisfile']." class=\"qnapics\">";
					}
				...
				if($xxt){
						...
						foreach($temp2 as $tt){
							if($notlist=share_gettable($pdo,"not_ WHERE memberid='".$tt['memberid']."'AND typeid=1 AND thislink='".$x[2]."' AND viewed=0")){//目前有資料
								...
								$sendcontent=$me['nickname']."也回覆了 ".$he['nickname']."的Q&A"; //20181228 Pman QA被回覆時，發出通知推播
								$others=share_getinfo($pdom,"mem_","memberid",$tt['memberid']);//20181228 Pman 針對同一篇有留言的其它人資料
								if($others['fcmid']){
									sendfcm($others['fcmid'],"Q&A回覆",$sendcontent,"",$others['note_count']+1,$others['mobtype']); //20181228 Pman QA被回覆時，發出通知推播 //20190220 Pman 新增"mobtype"參數
								}
						...
						foreach($temp2 as $tt){
							if($tx=share_gettable($pdo,"not_ WHERE memberid='".$tt['memberid']."' AND thislink='".$x[2]."' AND typeid=2 AND viewed=0")){//看有沒有沒看過的
								share_update($pdo,"not_","fromid='".$_SESSION['userid']."',typeid='2',thiscontent='".$me['nickname']."等人也回覆了 ".$he['nickname']."的Q&A'","thisid='".$tx[0]['thisid']."'");
							}else{//都看過了就加新的
								share_insert($pdo,"not_","memberid,fromid,typeid,thiscontent,thislink","'".$tt['memberid']."','".$_SESSION['userid']."',2,'".$me['nickname']."回覆了 ".$he['nickname']."的Q&A','".$x[2]."'");
							}
						}
	...
	function uploadartpagereply($x){
		...
		if($x[0]==$_SESSION['userid'] && $x[1]==$_SESSION['key'] && $x[2]<>""){//確認資格
			...
			if($x[5]=="1"){//圖片
				...
				$xxt=share_insert($pdo,"rep_","contentid,memberid,thiscontent,timekey","'".$x[2]."','".$_SESSION['userid']."','<div class=\"newstextbox\">".share_html($x[3])."</div>','".time()."'");
			}
			if($xxt){
	...
	function uploadnewsreply($x){
		global $conf;
		if($x[0]==$_SESSION['userid'] && $x[1]==$_SESSION['key'] && $x[2]<>"" && ($x[3]<>"" || $x[5]=="1")){//20190111 Pman 調整成回覆只要文字或圖片擇一即可
			$pdo = new PDO('mysql:host='.$conf['dbhost_d'].';dbname='.$conf['dbname_d'], $conf['dbuser_d'], $conf['dbpass_d']);
			$pdo -> exec("set names ".$conf['db_encode']);
			$pdom = new PDO('mysql:host='.$conf['dbhost_m'].';dbname='.$conf['dbname_m'], $conf['dbuser_m'], $conf['dbpass_m']);
			$pdom -> exec("set names ".$conf['db_encode']);
			...
			if($x[6]){
				...
				//if(strpos($x[3],$tempr['nickname'])==1){
				if($tempr){ //20190401 Pman 改用是否有找到這個id來判斷
					$rins="<span class=\"replytonamebox\">".$tempr['nickname']."</span>";
					//$x[3]=ltrim($x[3],"@".$tempr['nickname']); //20190401 Pman 因為前台輸入時，@XXXX已經不在輸入的內容中，所以將其移除
					//$x[3]=$rins.$x[3];
				}
			}
			if($x[5]=="1"){//圖片
			//	$xxt=share_insert($pdo,"rep_","contentid,memberid,thiscontent,timekey","'".$x[2]."','".$_SESSION['userid']."','<div class=\"newstextbox\">".share_html($x[3])."</div><div class=\"newsfilebox \"><img src='uploadfile/".$x[4]."'  ></div>','".time()."'");
				if($x[3]==""){//20190111 Pman 如果只有圖片沒有文字時，就不填入文字區塊的內容
					$xxt=share_insert($pdo,"rep_","contentid,memberid,thiscontent,timekey","'".$x[2]."','".$_SESSION['userid']."','<div class=\"newstextbox\"></div><div class=\" newsfilebox popimgclick newsrimg \"  data-type=\"chat\" ><br /><img src=\"uploadfile/".$x[4]."\" ></div>','".time()."'");
				}else{
					$xxt=share_insert($pdo,"rep_","contentid,memberid,thiscontent,timekey","'".$x[2]."','".$_SESSION['userid']."','<div class=\"newstextbox\">".$rins.share_html($x[3])."</div><div class=\" newsfilebox popimgclick newsrimg \"  data-type=\"chat\" ><br /><img src=\"uploadfile/".$x[4]."\" ></div>','".time()."'");//20180906 Pman 客戶要求要加一個空行
				}
			}else{//一般文章
					...
					if($temp1['memberid']==$_SESSION['userid']){
						...
						if($tx=share_gettable($pdo,"not_ WHERE memberid='".$temp1['memberid']."'AND typeid=1 AND thislink='".$x[2]."' AND viewed=0")){//看看有沒有回覆過的還沒看
							if($tx['fromid']==$_SESSION['userid']){//最新就是我..不用更新
								...
								share_update($pdo,"not_","fromid='".$_SESSION['userid']."',thiscontent='".$me['nickname']."等人回覆了 你的動態','");
							}
						}else{
							share_insert($pdo,"not_","memberid,fromid,typeid,thiscontent,thislink","'".$temp1['memberid']."','".$_SESSION['userid']."',1,'".$me['nickname']."回覆了 你的動態','".$x[2]."'");
						}
			...
			$pdo=null;
		}else if($x[0]==$_SESSION['userid'] && $x[1]==$_SESSION['key']){
			$out[0]="ERR";
			$out[1]="請填寫文字";
		}else{
			$out[0]="ERR";
	...
	function uploadarticletext($x){//4/22 修改新增編輯的條件
		...
		if($x[0]==$_SESSION['userid'] && $x[1]==$_SESSION['key'] && $x[2]<>""){//確認資格
			...
			if($chk==1){
				$ttx=$x[5];
				$ttx=str_replace("{image:", "<img src=uploadfile/",$ttx);
				$ttx=str_replace("}", ">",$ttx);
				//$x[5]="<p>".$ttx."</p>";
				$x[5]=$ttx;
				$temp="";
	...
	function uploadnewsin($x){//更新動態牆內容
		...
		if($x[0]==$_SESSION['userid'] && $x[1]==$_SESSION['key']){//確認資格
			...
			if(share_update($pdo,"con_","thiscontent='<div class=\"newstextbox\">".share_html($x[2]).$tttb."'","thisid='".$x[3]."' AND memberid='".$_SESSION['userid']."'")){
					$out[0]="OK";
			}else{
					$out[0]="ERR";
					$out[1]="存入錯誤,請重新試試,謝謝";
	...
	function uploadalbumtext($x){//修改相簿文字
		//確認權限
		global $conf;
		$pdo = new PDO('mysql:host='.$conf['dbhost_d'].';dbname='.$conf['dbname_d'], $conf['dbuser_d'], $conf['dbpass_d']);
		$pdo -> exec("set names ".$conf['db_encode']);
		if($t=share_getinfo($pdo,"alb_","thisid",$x[4])){
			if($t['memberid']==$_SESSION['userid'] && $x[1]==$_SESSION['key']){
				share_update($pdo,"alb_","thisname='".$x[2]."',thiscontent='".$x[3]."'","thisid='".$x[4]."'");
				$out[0]="OK";
			}else{
				$out[0]="ERR";
				$out[1]="沒有修改的權限或登入已過期,請重新試試,謝謝";
			}
		}else{
					$out[0]="ERR";
					$out[1]="相簿不存在";
			...
			if( $x[4] && $x[5]=="1"){//圖片
				...
				if($ttotal>0){
					...
					*/
					...
					if($x[3]){//一般文章
						if($x[2]){
							if(share_update($pdo,"con_","typeid='".$x[5]."',gamid='".$x[6]."',opentype='".$x[7]."',thiscontent='<div class=\"newstextbox\">".share_html($x[3])."</div>'","thisid='".$x[2]."'")){
								//20190322 Pman 修正更新動態牆文章，沒有更新到wall裡面的opentype
								if(share_update($pdo,"wall","opentype='".$x[7]."'","thisid='".$x[2]."'")){
									$out[0]="OK";
								}
							}else{
								$out[0]="ERR";
				...
				if($x[2]){
					if(share_update($pdo,"con_","typeid='".$x[5]."',gamid='".$x[6]."',opentype='".$x[7]."',thiscontent='<div class=\"newstextbox\">".share_html($x[3])."\n</div><div class=\"newsfilebox\"><video width=100% controls id=newstempvideo><source src=uploadfile/".$x[4]." type=video/mp4 ></video></div>',fileinfo='".$x[4]."'","thisid='".$x[2]."'")){
						$out[0]="OK";
					}else{
						$out[0]="ERR";
						$out[1]="存入錯誤,請重新試試,謝謝";
					...
					if(share_insert($pdo,"con_","memberid,typeid,gamid,opentype,thiscontent,timekey,fileinfo","'".$_SESSION['userid']."','".$x[5]."','".$x[6]."','".$x[7]."','<div class=\"newstextbox\">".share_html($x[3])."\n</div><div class=\"newsfilebox\"><video width=100% controls id=newstempvideo><source src=uploadfile/".$x[4]." type=video/mp4 ></video></div>','".time()."','".$x[4]."'")){
						$t=share_gettable($pdo,"con_ WHERE memberid='".$_SESSION['userid']."' order by thisid DESC limit 1");
	...
	function uploadhead($x){
		$id=rand(123,987).date('Yndhms').rand(123,987);
		$t=uploadfilebase($id,600,2,600,$_FILES["val"],100,100);
		echo $id.".".$t;
	...
	function uploadarticleinsert($x){
		$id=rand(123,987).date('Yndhms').rand(123,987);
		$t=uploadfilebase($id,700,0,0,$_FILES["val"]);//$t=uploadfilebase($id,229,0,129,$_FILES["val"]);
	...
	function uploadnewspicbook($x){//上船相簿圖片--這是一開始上傳的uploadinstantm
		global $conf;
	...
	function uploadnewspic($x){//上傳非相簿的相片
		global $conf;
		$pdod = new PDO('mysql:host='.$conf['dbhost_d'].';dbname='.$conf['dbname_d'], $conf['dbuser_d'], $conf['dbpass_d']);
		$pdod -> exec("set names ".$conf['db_encode']);
		if($_SESSION['userid']==$GLOBALS['uid'] && $_SESSION['key']==$GLOBALS['ukey']){
	...
	function uploadnewspicdel($x){//動態/相簿內相片刪除
		global $conf;
		$pdod = new PDO('mysql:host='.$conf['dbhost_d'].';dbname='.$conf['dbname_d'], $conf['dbuser_d'], $conf['dbpass_d']);
		$pdod -> exec("set names ".$conf['db_encode']);
		if($_SESSION['userid']==$x[0] && $_SESSION['key']==$x[1]){
			...
			$tem2=share_gettable($pdod,"pho_","albid",$tem1['albid']);
			share_del($pdod,"pho_ WHERE memberid='".$_SESSION['userid']."' AND thisfile='".$x[2]."'");
			if(count($tem2)>1){//還有
				//share_del($pdod,"alb_ WHERE memberid='".$_SESSION['userid']."' AND thisid='".$tem1['albid']."'"); //20181102 Pman 修正在動態牆上傳照片時，如果有刪除任何一張，就會讓相簿編號消失，以至於送出後無法點開觀看的問題。
				$out[0]="OKA";
			}else{
				$out[0]="ERR";
			}
	...
	function uploadalbpicdel($x){//動態/相簿內相片刪除
		global $conf;
		$pdod = new PDO('mysql:host='.$conf['dbhost_d'].';dbname='.$conf['dbname_d'], $conf['dbuser_d'], $conf['dbpass_d']);
		$pdod -> exec("set names ".$conf['db_encode']);
		if($_SESSION['userid']==$x[0] && $_SESSION['key']==$x[1]){
		...
		if($_SESSION['userid']==$GLOBALS['uid'] && $_SESSION['key']==$GLOBALS['ukey']){
			...
			if($tpy['isdefault']>=1){//一班上傳
				if($tpy['isdefault']>1 && $cnt>=20){
					...
					echo $id.".".$t."_".$cnt."_".$GLOBALS['albid'];
				}
			}else{
				if($cnt>20){
					echo "ERRC";
	...
	function addalbpicx($x){//增加相簿相片
		...
		if($_SESSION['userid']==$GLOBALS['uid'] && $_SESSION['key']==$GLOBALS['ukey']){
			...
			//這裡只有相簿類型
			/*
			if($tpy['isdefault']>=1){//一班上傳....相簿類別
				if($cnt>=5){
					echo "ERRB";
				}else{
					$id=rand(123,987).date('Yndhms').rand(123,987);
					$t=uploadfilebase($id,1900,1,'',$_FILES["val"]);
					share_insert($pdod,"pho_","memberid,albid,thisfile","'".$_SESSION['userid']."','".$GLOBALS['albid']."','".$id.".".$t."'");
					echo $id.".".$t."_".$cnt;
				}
			}else{
	...
	function uploadfilebase($id,$x,$y,$h,$fil,$tw,$th){//使用者上傳	ID,寬,是否強迫高寬,高度,檔案
		//$file=$_FILES["val"];
		$file=$fil;
		$imgwidth="";
		$imgurl="../uploadfile/";
		...
		if(($file["type"] == "image/jpeg") || ($file == "image/pjpeg")){
			$fileext="jpg";
