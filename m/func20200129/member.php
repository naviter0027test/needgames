<?php
	...
	function mem_forget($x){
		...
		$pdo = new PDO('mysql:host='.$conf['dbhost_m'].';dbname='.$conf['dbname_m'], $conf['dbuser_m'], $conf['dbpass_m']);
		$pdo -> exec("set names ".$conf['db_encode']);
		if($t=share_getinfo($pdo ,"mem_","email",$x[1])){
				$out[0]="OK";
	...
	//FB快速註冊
	function mem_fbreg($x){
		global $conf;
		$out="";
		$pdom = new PDO('mysql:host='.$conf['dbhost_m'].';dbname='.$conf['dbname_m'], $conf['dbuser_m'], $conf['dbpass_m']);
		$pdom -> exec("set names ".$conf['db_encode']);
		if($t=share_getinfo($pdom ,"mem_","fbid",$x[0])){
			$out[0]="ERR";
			$out[1]="對不起,這個facebook帳號已經註冊，請選用其他帳號";
		}else{
			$fcname="";
			$fcmid="";
			if($x[4]){
				...
				$fcmid=",'".$x[4]."','".$x[5]."'";
				share_update($pdom,"mem_","fcmid=null,mobtype=null","fcmid='".$x[4]."'");
			}
			if(share_insert($pdom ,"mem_","fbid,fbname,fbmail,fbbirth,actcode".$fcname,"'".$x[0]."','".$x[1]."','".$x[2]."','".$x[3]."','1'".$fcmid)){
				$_SESSION['fbid']=$x[0];
				$out[0]="OK";
				$tb=share_getinfo($pdom ,"mem_","fbid",$x[0]);//重新抓取]
				if($x[6]){//有人介紹
					$myinfo=$tb;
					$fid=($x[6]-10000000000000)/13;
					$refinfo=share_getinfo($pdom ,"mem_","memberid",$fid);
					$mid=10000000000000+$myinfo['memberid']*13;
					share_insert($pdom ,"reflist","refid,refname,memberid,memname,regtime,isexport","'".$x[6]."','".$refinfo['nickname']."','".$mid."','".$myinfo['nickname']."','".$myinfo['dateadd']."','0'");
				}
				for($x=0;$x<count($tb);$x++){
					unset($tb[$x]);
				}
	...
	//email 重新綁定
	function mem_reemail_send($x){
		global $conf;
		$out="";
		$pdo = new PDO('mysql:host='.$conf['dbhost_m'].';dbname='.$conf['dbname_m'], $conf['dbuser_m'], $conf['dbpass_m']);
		$pdo -> exec("set names ".$conf['db_encode']);
		$t=share_getinfo($pdo ,"mem_","memberid",$_SESSION['userid']);
		if($t['email']==$x[0]){
			$out[0]="ERR";
			$out[1]="Email沒有改變不需要重新綁定,謝謝";
		}else{
			if($tt=share_gettable($pdo ,"mem_ WHERE email='".$x[0]."'")){
				$out[0]="ERR";
				$out[1]="本Email已有其他帳號綁定,請選擇其他Email,謝謝";
			}else{
				$_SESSION['vercode']=rand(123456,987654);
				$_SESSION['veremail']=$x[0];
				sendmail('5',$x[0],$_SESSION['vercode']);
				$out[0]="OK";
				$out[1]="我們已經將驗證碼寄至您的信箱,請前往取得,謝謝";
			}
		}
		echo json_encode($out);
	}

	function mem_reemail_chk($x){
		global $conf;
		$out="";
		if($x[0]==$_SESSION['vercode']){
			$pdo = new PDO('mysql:host='.$conf['dbhost_m'].';dbname='.$conf['dbname_m'], $conf['dbuser_m'], $conf['dbpass_m']);
			$pdo -> exec("set names ".$conf['db_encode']);
			share_update($pdo ,"mem_","email='".$_SESSION['veremail']."'","memberid='".$_SESSION['userid']."'");
			$out[0]="OK";
		}else{
			$out[0]="ERR";
			$out[1]="驗證碼錯誤，請確認或重新寄送，謝謝";
		}
		echo json_encode($out);
	}
	//登入
	...
	function mem_logoff($x){
		if($x[0]=="app"){//移除fcmid
			...
			$pdo = new PDO('mysql:host='.$conf['dbhost_m'].';dbname='.$conf['dbname_m'], $conf['dbuser_m'], $conf['dbpass_m']);
			$pdo -> exec("set names ".$conf['db_encode']);
			$tx=share_getinfo($pdo,"mem_","memberid",$_SESSION['userid']);
			if($tx['fcmid']){
				fcmupdatebadge($tx['fcmid'],0);
			}
			share_update($pdo,"mem_","fcmid=null,mobtype=null","memberid='".$_SESSION['userid']."'");
		}
			...
			if((($x[1]-1358743953456)/1357531)==$x[0]){//正確
				...
				if($t['actcode']==1){
					if($t['locker']==1){
						...
						if(!empty($x[2])){
						//	share_update($pdo,"mem_","lastIP='".GetIP()."',lastlogin=NOW(),'".$x[2]."'","memberid='".$t['memberid']."'"); ///這是舊版--不知道哪裡有用 2019/1/07
						share_update($pdo,"mem_","lastIP='".GetIP()."',lastlogin=NOW(),fcmid='".$x[2]."',mobtype='".$x[3]."'","memberid='".$t['memberid']."'");
						}else{
							share_update($pdo,"mem_","lastIP='".GetIP()."',lastlogin=NOW()","memberid='".$t['memberid']."'");
						}
						$temp=$mrr[rand(0,21)].$mrr[rand(0,21)].rand(12,98).$mrr[rand(0,21)].rand(12,98).$mrr[rand(0,21)].rand(12,98).$mrr[rand(0,21)].$mrr[rand(0,21)].$mrr[rand(0,21)];
	...
	function chk_mem($x){
		if($_SESSION['userid'] && $_SESSION['key'] && $_SESSION['userid'] && $x[0]==$_SESSION['userid'] && $x[1]==$_SESSION['key']){
			$out[0]="OK";
		}else{
			$out[0]="ERR";
		}
	...
	function mem_useredit($x){
		...
		if($_SESSION['userid']==$x[0] && $_SESSION['key']==$x[1]){
			...
			//	if($tb=share_gettable($pdo ," mem_ WHERE nickname='".$x[2]."' AND memberid<>'".$_SESSION['userid']."'")){//檢查email
			//		$out[0]="ERR";
			//		$out[1]="這個暱稱已被其他會員使用,請選用其他暱稱";
			//	}else{
					//if(share_update($pdo ,"mem_","gender='".$x[3]."',nickname='".$x[2]."',birthday='".$x[4]."-".$x[5]."-".$x[6]."',location='".$x[7]."'".$ins.",gtid='".$x[14]."',headpic='".$x[15]."',lastIP='".GetIP()."'","memberid='".$_SESSION['userid']."'")){
	...
	function chk_notice($x){
		//確認聊天室
		chat_chk_note();
		$note_count=6; //20181224 Pman 儲存通知的總數
		global $conf;
		if($x[0]==$_SESSION['userid'] && $x[1]==$_SESSION['key']){//確認資格
			$out[0]="OK";
			...
			$pdoc=null;
			$pdod=null;
			//20181224 Pman 將通知的總數存入mem_資料表中
			$pdoe = new PDO('mysql:host='.$conf['dbhost_m'].';dbname='.$conf['dbname_m'], $conf['dbuser_m'], $conf['dbpass_m'] );
			$pdoe -> exec("set names ".$conf['db_encode']);
			$note_count=$out[0]+$out[1]+$out[2];
			share_update($pdoe ,"mem_","note_count='".$note_count."'","memberid='".$_SESSION['userid']."'");
			$pdoe=null;
			//============================================
			//20190508 Pman 檢查帳號是否被鎖定
			$pdom = new PDO('mysql:host='.$conf['dbhost_m'].';dbname='.$conf['dbname_m'], $conf['dbuser_m'], $conf['dbpass_m']);
			$pdom -> exec("set names ".$conf['db_encode']);
			$tb=share_getinfo($pdom,"mem_","memberid",$x[0]);
			//$tb[41]
			//lockertime<CURDATE()
			//$out[2]=date("Y-m-d");
			$lock_time=strtotime($tb[42]);
			$_today=strtotime(date("Y-m-d"));

			//if($tb[41]==2 && $lock_time>=$_today){
			if($tb[41]==2){ //20190509 Pman 修正鎖定的判定方式
				$out[0]="ERROUT";
			}
			$pdom=null;
