	...
	if(isapp){
		...
		document.addEventListener("deviceready",function(){
			StatusBar.overlaysWebView(false);StatusBar.backgroundColorByHexString("#222");
...
//    }
   ...
   $("body").delegate(".alert li .sub","click",function() {
        $(this).toggleClass("on");
    });
		// ################  TAB 切換  ##################################
	...
	showpage=function(x,y){
		if(window.page=="share.php"){
			...
			if(x==99){//登入頁
				...
				$("footer").show();
				if(changef==1){
					$('footer .slides').css("width",$window.width()-130);
					setTimeout(function(){
						$('footer .slides').slick();
					},800);
				}
